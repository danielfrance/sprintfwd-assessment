<x-app-layout>
    <div class="mx-auto mt-8 px-4">
        <div class="flex items-center justify-between">
            <h3 class="mb-4 mt-4 text-4xl text-gray-500 dark:text-gray-200 font-extrabold">Members</h3>

            <div>
                <x-primary-button x-data="" x-on:click="$dispatch('open-modal','new-member-modal')">
                    {{ __('New Member') }}
                </x-primary-button>
        ...
        <div class="w-full">
             <div class="relative">
                <input
                    ...
                    name="search" type="text" placeholder="Search Members" />
            </div>
    ...
    <x-modal name="new-member-modal" focusable class="bg-gray-500">
        <form method="post" action="{{ route('members.store') }}" enctype="multipart/form-data" class="p-6">
            @csrf

            <h3 class="mb-4 mt-4 text-3xl text-gray-500 font-extrabold dark:text-gray-200">Create Member</h3>

            <div class="grid grid-cols-2 gap-4">
                <div>
                    <label class="block mb-2 text-lg font-medium text-gray-900 dark:text-gray-200" for="first_name">First Name</label>
                    <input
                        class="block w-full text-lg text-gray-900  border border-gray-300 rounded-lg cursor-pointer  focus:outline-none "
                        name="first_name" type="text">
                </div>
                 <div>
                    <label class="block mb-2 text-lg font-medium text-gray-900 dark:text-gray-200" for="last_name">Last Name</label>
                    <input
                        class="block w-full text-lg text-gray-900  border border-gray-300 rounded-lg cursor-pointer  focus:outline-none "
                        name="last_name" type="text">
                </div>
                <div>
                    <label class="block mb-2 text-lg font-medium text-gray-900 dark:text-gray-200" for="team">Team</label>
                    <select name="team" id="team"
                        class="block w-full text-lg text-gray-900 border border-gray-300 rounded-lg cursor-pointer  focus:outline-none ">
                        <option value="" selected disabled>Assign a Team</option>
